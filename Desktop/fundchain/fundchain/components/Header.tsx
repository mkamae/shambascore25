...
const Header: React.FC = () => {
  const { isConnected, principal, connect, disconnect, balance } = usePlugWallet();
          ...
          <div className="hidden md:flex items-center">
            {isConnected && principal ? (
              <div className="flex items-center gap-4">
                  ...
                  <span className="text-xs text-brand-400 bg-slate-800 px-2 py-1 rounded-md ml-2">{balance !== null ? `${balance.toFixed(4)} ICP` : '...'}</span>
                  <button onClick={disconnect} className="text-slate-400 hover:text-white text-sm">Logout</button>
              </div>
            ) : (
              <button
                onClick={connect}
                className="flex items-center gap-2 bg-brand-500 hover:bg-brand-600 text-white font-bold py-2 px-4 rounded-md transition duration-300 ease-in-out"
              >
                ...
                <div className="pt-4 mt-4 border-t border-slate-700">
                 {isConnected && principal ? (
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center gap-2 bg-brand-500 hover:bg-brand-600 text-white font-bold py-2 px-4 rounded-md transition duration-300 ease-in-out"
                  >
                    <WalletIcon className="h-5 w-5" />
                    Connect Plug Wallet
                  </button>
